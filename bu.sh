...
# Intercept 'autocomplete' subcommand — click ref, type text, pick option
# Works with React Select, Select2, MUI Autocomplete, and similar typeahead widgets
if [ "$1" = "autocomplete" ]; then
  shift
  exec uv run --directory "$SKILL_DIR" python "$SKILL_DIR/autocomplete.py" "${BU_FLAGS[@]}" "$@"
fi
